...
The initialization of the routing rules based on the topology and the pipeline and it continues learning and computing routing rules through processing new packets. We will decribe this part further later.

\projectcode2026-1-1\log
Record the working process of the whole system. Initialization and working...

\projectcode2026-1-1\pcap
Record the packets information on single switch port.
...
That means the dst peswitch is the src peswitch. It tries to use the dmac forward to 'dst host' or broadcast to 'dst host'. 
...
So the packets may be in the tunnel or at the dst peswitch. If it is in the tunnel, it will do the vpls_tbl forward from one hop to next hop based on the ingress and egress port.
If it is at the dst peswitch, it will do the vpls_dmac forward (the dst host known) or vpls_dmac_broadcast (unknown).
...
between different peswitches which have same pw_id host. We record the ports between the src host and the src peswitch with pw_id, 
the ports between the src peswitch and the next hop switch in tunnel in dict. We also classify these ports into different groups for broadcast or ecmp group. 
...
The src peswitch will learn the src mac addr and the input port and write the dmac and vpls_dmac rule in this peswitch. Second, the packets arrives the dst peswitch from the tunnels.
The dst peswitch will learn the src mac addr and write the rule of "addr_to_label" plus "ecmp_group" (multipaths) or "addr_to_label" (single path) using this src mac addr as the dst mac addr.
